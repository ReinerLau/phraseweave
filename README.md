...
### 3. Configure Local Logto

Start the local Logto service with Docker Compose, then configure the local client values in `apps/api/.env` and `apps/client/.env`. Use locally generated credentials only; do not commit secrets or database snapshots.
...
Thanks to everyone who has already contributed to PhraseWeave! 🎉

Contributions are tracked in the private project repository.
